...
public:
    static void astar_multi_start_search_batch(const std::vector<std::vector<std::pair<int,value_t>>>& queries,int k,std::vector<std::vector<idx_t>>& results,value_t* h_data,idx_t* h_graph,int vertex_offset_shift,int num,int dim){
        ...
        // num= number of data points in the space
        // dim= dimesion of the space
        // (2^vertex_offset_shift)= Max no of neighbours
        // value_t= float
        // idx_t= size_t
        ...
        auto t_memtransfer= std::chrono::duration_cast<std::chrono::microseconds>(mem_transfer - time_begin).count();
        fprintf(stderr,"mem transfer takes: %ld microseconds\n", t_memtransfer);
        fprintf(stdout,"mem transfer takes: %ld microseconds\n", t_memtransfer);
        ...
        auto t_kernel =std::chrono::duration_cast<std::chrono::microseconds>(kernel_end - kernel_begin).count();
        fprintf(stderr,"kernel takes: %ld microseconds\n",t_kernel);
        fprintf(stdout,"kernel takes: %ld microseconds\n",t_kernel);
        ...
        auto t_transfer= std::chrono::duration_cast<std::chrono::microseconds>(back_end - back_begin).count();
        fprintf(stderr,"transfer back result takes: %ld microseconds\n", t_transfer);
        fprintf(stdout,"transfer back result takes: %ld microseconds\n", t_transfer);
